...
show_usage () {
    ...
    echo "    --osk-command-config [osk-command-config-file]"
    echo "    --ccloud-bootstrap-server [ccloud-bootstrap-server-list]"
    echo "    --ccloud-command-config [ccloud-command-config-file]"
...
do
    ...
    then
        ...
        OSK_COMMAND_CONFIG="$2"
        echo "OSK Command Config File path is: ${OSK_COMMAND_CONFIG}"
...
if [[ -z "$OSK_BOOTSTRAP_SERVERS"  ]] || [[ -z "$CCLOUD_BOOTSTRAP_SERVERS"  ]] || [[ -z "$CCLOUD_COMMAND_CONFIG"  ]] || [[ -z "$OSK_COMMAND_CONFIG"  ]]
then
    echo "--osk-bootstrap-server, --osk-command-config, --ccloud-bootstrap-server, and --ccloud-command-config are required for execution."
    show_usage
    exit 1
fi
...
kafka-consumer-groups --bootstrap-server ${OSK_BOOTSTRAP_SERVERS} --command-config ${OSK_COMMAND_CONFIG} --list > ${CONSUMER_GROUPS}
...
do
    ...
    then
        ...
        kafka-consumer-groups --bootstrap-server ${OSK_BOOTSTRAP_SERVERS} --command-config ${OSK_COMMAND_CONFIG} --group ${group_name} --describe > ${OSK_GROUP_DATA}
        kafka-consumer-groups --bootstrap-server ${CCLOUD_BOOTSTRAP_SERVERS} --command-config ${CCLOUD_COMMAND_CONFIG} --group ${group_name} --describe -timeout 10000 > ${CCLOUD_GROUP_DATA}
